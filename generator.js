...
"ZW - Unicorn Spear",
    ...
    const FORBIDDEN_TOKENS = new Set(['ofthe']);
    ...
    const FORBIDDEN_PATTERNS = {
        ...
        doubleApostrophes: /''/,
        doubleComma: /,,/,
        badDashSpacing: /-[ ]+[^A-Za-z]|-[^A-Za-z\s]/,
        dashSpace: /\w-\s+\w/,
        lowercaseAbbrev: /\b[a-z]\.[a-z]\.?/i, 
        exclamationComma: /!,/,
        commaS: /,'s/,
        commaS: /s'[ ]-/,
        dotcom: /.,/,
        commaSS: /s's/,
        possessiveConnector: /'s\s+(of|the|and|but|or|in|on|at|by|for|with|from|a|an|as|if|into|is|it|no|not|such|that|their|then|there|these|they|this|to|was|will)\b/i,
        mostlyNumbers: /^\d+\s*\w{0,2}$/,
        excessivePunctuation: /[!@#$%^&*()_+=\[\]{}|\\:";'<>?,./]{3,}/,
        badConnectorStart: /^(of|and|but|or|in|on|at|by|for|with|from)\b/i,
        badConnectorEnd: /\b(of|and|but|or|in|on|at|by|for|with|from)$/i,
        connectorAfterPunctuation: /[.:,'"\-]\s*(of|the|and|but|or|in|on|at|by|for|with|from|a|an|as|if|into|is|it|no|not|such|that|their|then|there|these|they|this|to|was|will)\b/i,
        duplicateConnectors: /\b(the|of|and|but|or|in|on|at|by|for|with|from|a|an|as|if|into|is|it|no|not|such|that|their|then|there|these|they|this|to|was|will)\s+\1\b/i,
        connectorPossessive: /\b(of|the|and|but|or|in|on|at|by|for|with|from|a|an|as|if|into|is|it|no|not|such|that|their|then|there|these|they|this|to|was|will)'s\b/i,
    };
    ...
    const THEMES = {
        dragons: ['dragon', 'drake', 'wyrm', 'wyvern', 'dragunity'],
        dragonmaids: ['dragonmaid'], 
        warriors: ['warrior', 'knight', 'soldier', 'paladin', 'hero', 'elemental HERO'],
        magic: ['magician', 'wizard', 'sorcerer', 'spellcaster', 'witch'],
        elements: ['fire', 'water', 'earth', 'wind', 'light', 'dark', 'shadow'],
        celestial: ['star', 'moon', 'sun', 'cosmic', 'galaxy', 'astral'],
        machine: ['machine', 'gear', 'mech', 'robot', 'cyber'],
        fiend: ['fiend', 'demon', 'devil', 'archfiend', 'daemon'],
        beast: ['beast', 'animal', 'creature', 'feral']
    };

    const PREFIXES = ["Ultimate", "Supreme", "Neo", "Ancient", "Chaos", "Divine", "Elemental", "Mystic", "Phantom", "Sacred"];
    const SUFFIXES = ["King", "Queen", "Lord", "Master", "Emperor", "Guardian", "Protector", "Destroyer", "Keeper", "Overlord"];
    const JOINERS = [", the", " of", " the", "'s", "-"];

    // =============================================================================
    // STATE
    // =============================================================================
    
    let cardNames = [...SAMPLE_CARD_NAMES];
    let currentMethod = 'wordSplit';
    let generatedNamesSet = new Set();
    let markovCache = { order2: null, order1: null, lastDataLength: 0 };
    ...
    const elements = {
        ...
        numNames: document.getElementById('numNames'),
    ...
    function containsForbiddenToken(name, isAlreadyFormatted = false) {
        const testName = isAlreadyFormatted ? name : formatCardName(name);
        const lowerName = testName.toLowerCase();
        
        // Check forbidden tokens
        const words = lowerName.split(/\s+/);
        if (words.some(word => FORBIDDEN_TOKENS.has(word.replace(/[^a-z]/g, '')))) {
            return true;
        }
        
        // Check all forbidden patterns
        return Object.values(FORBIDDEN_PATTERNS).some(pattern => pattern.test(testName));
    }
    ...
    function isTooSimilarToSample(name, sampleNames) {
        const lowerName = name.toLowerCase();
        return sampleNames.some(sample => {
            const dist = levenshtein(lowerName, sample.toLowerCase());
            const similarity = 1 - dist / Math.max(lowerName.length, sample.length);
            return similarity >= CONFIG.similarityThreshold;
        });
    }

    function isValidName(name) {
        if (!name || name.length < CONFIG.minNameLength || name.trim() === '') {
            return false;
        }

        // Format the name first before checking patterns
        const formattedName = formatCardName(name);

        if (cardNames.some(existing => existing.toLowerCase() === formattedName.toLowerCase()) ||
            isTooSimilarToSample(formattedName, cardNames) ||
            generatedNamesSet.has(formattedName.toLowerCase()) ||
            containsForbiddenToken(formattedName, true)) {
            return false;
        }
    ...
    function applyCaseFixes(name) {
        // Apply all case fixes in sequence
        Object.entries(CASE_FIXES.fixedDots).forEach(([lower, correct]) => {
            const regex = new RegExp(`\\b${lower}\\b`, 'gi');
            name = name.replace(regex, correct);
        });

        Object.entries(CASE_FIXES.fixedColons).forEach(([term, correct]) => {
            const regex = new RegExp(`\\b${term}\\b`, 'gi');
            name = name.replace(regex, correct);
        });

        Object.entries(CASE_FIXES.normalized).forEach(([term, correct]) => {
            const regex = new RegExp(`\\b${term}\\b`, 'g');
            name = name.replace(regex, correct);
        });

        // Fix CXyz prefix
        name = name.replace(/\bCxyz(?=[\s\-]|$)/gi, 'CXyz');
    ...
    function formatCardName(name) {
        ...
        const formattedWords = words.map((word, index) => {
            ...
            // Check if it's a connecting word that should stay lowercase
        ...
        // Ensure letters after hyphens are capitalized
        result = result.replace(/-([a-z])/g, (_, char) => '-' + char.toUpperCase());
        
        return applyCaseFixes(result);
    }
    ...
    function getRandomWords(cardName) {
        return cardName.split(' ').filter(w => w.length > 0);
    }

    function generateByWordSplit() {
        const card1 = getRandomElement(cardNames);
        const card2 = getRandomElement(cardNames);
        
        const words1 = getRandomWords(card1);
        const words2 = getRandomWords(card2);
        
        if (words1.length === 0 || words2.length === 0) {
            return 'Unknown Card';
        }
        
        const takeFromFirst = Math.max(1, Math.floor(Math.random() * words1.length));
        let newName = words1.slice(0, takeFromFirst).join(' ');
        
        // Randomly add a joiner
        if (Math.random() > 0.7) {
            newName += getRandomElement(JOINERS);
        }
        
        const takeFromSecond = Math.max(1, Math.floor(Math.random() * words2.length));
        newName += ' ' + words2.slice(-takeFromSecond).join(' ');
        
        return formatCardName(newName.replace(/\s+/g, ' ').trim());
    }
    
    function generateBySyllableSplit() {
        const card1 = getRandomElement(cardNames);
        const card2 = getRandomElement(cardNames);
        
        function roughSplit(name) {
            return name.replace(/([aeiou][^aeiou])/gi, '$1#').split('#').filter(part => part.length > 0);
        }
        
        const parts1 = roughSplit(card1.replace(/\s+/g, ''));
        const parts2 = roughSplit(card2.replace(/\s+/g, ''));
        
        if (parts1.length === 0 || parts2.length === 0) {
            return generateByWordSplit();
        }
        
        const cutPoint1 = Math.max(1, Math.floor(parts1.length / 2));
        const cutPoint2 = Math.max(1, Math.floor(parts2.length / 2));
        
        const newName = Math.random() > 0.5 
            ? parts1.slice(0, cutPoint1).join('') + parts2.slice(cutPoint2).join('')
            : parts2.slice(0, cutPoint2).join('') + parts1.slice(cutPoint1).join('');
        
        return formatCardName(newName);
    }
    
    function generateByThematic() {
        // Group cards by themes
        const themeGroups = {};
        Object.keys(THEMES).forEach(theme => {
            themeGroups[theme] = cardNames.filter(card => 
                THEMES[theme].some(keyword => 
                    card.toLowerCase().includes(keyword)
                )
            );
        });
        
        const availableThemes = Object.keys(themeGroups).filter(theme => 
        ...
        const selectedTheme = getRandomElement(availableThemes);
        const themeCards = themeGroups[selectedTheme];
        const card1 = getRandomElement(themeCards);
        const card2 = getRandomElement(themeCards);
        ...
        switch(strategy) {
            case 0: // Prefix + part of card
                newName = `${getRandomElement(PREFIXES)} ${getRandomElement(words1)}`;
                break;
            case 1: // Part of card + suffix
                newName = `${getRandomElement(words1)} ${getRandomElement(SUFFIXES)}`;
                break;
            case 2: // Parts from both cards
                newName = `${getRandomElement(words1)} ${getRandomElement(words2)}`;
                break;
            case 3: // Complex combination
                const prefix = Math.random() > 0.5 ? `${getRandomElement(PREFIXES)} ` : "";
                const suffix = Math.random() > 0.5 ? ` ${getRandomElement(SUFFIXES)}` : "";
                newName = `${prefix}${getRandomElement(words1)} ${getRandomElement(words2)}${suffix}`;
                break;
        }
        
        return formatCardName(newName.trim());
    }

    // =============================================================================
// IMPROVED MARKOV CHAIN FUNCTIONS
// =============================================================================

// Enhanced configuration with better linguistic awareness
const ENHANCED_MARKOV_CONFIG = {
    // Quality thresholds
    minQualityScore: 60,
    excellentQualityScore: 80,
    ...
    // Character pattern control
    maxConsecutiveConsonants: 3,
    maxConsecutiveVowels: 2,
    maxRepeatedChars: 2,
    
    // Linguistic patterns
    commonWordEndings: ['er', 'ed', 'ing', 'ion', 'tion', 'sion', 'ness', 'ment', 'ful', 'less', 'able', 'ible', 'ous', 'ic', 'al', 'ly'],
    commonWordStarts: ['un', 're', 'pre', 'dis', 'over', 'under', 'out', 'sub', 'super', 'anti', 'auto', 'co', 'ex', 'inter', 'multi'],
    
    // Yu-Gi-Oh specific patterns
    cardNamePatterns: {
        titleWords: ['lord', 'king', 'queen', 'master', 'guardian', 'keeper', 'knight', 'warrior', 'dragon', 'beast', 'elemental'],
        descriptors: ['ancient', 'mystic', 'dark', 'light', 'chaos', 'divine', 'sacred', 'shadow', 'crystal', 'neo', 'ultimate'],
        connectors: ['of', 'the', 'and', 'in', 'from', 'with']
    },
    
    // Forbidden patterns (more comprehensive)
    forbiddenPatterns: [
        /^[^a-zA-Z]/, // Must start with letter
        /[^a-zA-Z0-9\s\-']/, // Only allow alphanumeric, spaces, hyphens, apostrophes
        /\s{2,}/, // No multiple spaces
        /--/, // No double hyphens
        /'{2,}/, // No multiple apostrophes
        /^(the|a|an|of|and|or|but|in|on|at|by|for|with|from)$/i, // Can't be just connector words
        /(.)\1{3,}/, // No more than 3 consecutive identical characters
    ]
};

// Advanced word-level Markov chain with context awareness
function buildAdvancedWordChain(names, order = 2) {
    const chain = new Map();
    const startSequences = [];
    const wordFrequency = new Map();
    const positionStats = new Map(); // Track where words typically appear
    ...
    names.forEach(name => {
        ...
        words.forEach((word, index) => {
            ...
            if (!positionStats.has(cleanWord)) {
                positionStats.set(cleanWord, { start: 0, middle: 0, end: 0, total: 0 });
            }
            
            const stats = positionStats.get(cleanWord);
            stats.total++;
            
            if (index === 0) stats.start++;
            else if (index === words.length - 1) stats.end++;
            else stats.middle++;
        });
        
        // Build n-gram chains
        for (let i = 0; i <= words.length - order; i++) {
            const sequence = words.slice(i, i + order);
            const nextWord = i + order < words.length ? words[i + order] : '<END>';
            
            const key = sequence.join('|').toLowerCase();
            
            if (!chain.has(key)) {
                chain.set(key, new Map());
            }
            
            const transitions = chain.get(key);
            transitions.set(nextWord, (transitions.get(nextWord) || 0) + 1);
        }
        
        // Add start sequences
        if (words.length >= order) {
            startSequences.push(words.slice(0, order));
        }
    });
    
    return { chain, startSequences, wordFrequency, positionStats, order };
}

// Smart character-level chain with syllable awareness
function buildSmartCharacterChain(names, order = 3) {
    const chain = new Map();
    const startTokens = [];
    const syllablePatterns = new Map();
    
    names.forEach(name => {
        // Clean and prepare the name
        const cleanName = name.replace(/[^a-zA-Z\s]/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();
        if (cleanName.length < 3) return;
        
        // Track syllable-like patterns
        const syllables = cleanName.split(/\s+/).map(word => 
            word.replace(/([aeiou]+[bcdfghjklmnpqrstvwxyz]*)/g, '|$1').split('|').filter(s => s.length > 0)
        ).flat();
        
        syllables.forEach(syllable => {
            if (syllable.length >= 2) {
                syllablePatterns.set(syllable, (syllablePatterns.get(syllable) || 0) + 1);
            }
        });
        
        // Build character chain with word boundaries
        const words = cleanName.split(' ');
        words.forEach(word => {
            const paddedWord = '^'.repeat(order) + word + '$';
            
            // Collect start tokens
            if (paddedWord.length > order) {
                startTokens.push(paddedWord.substring(0, order));
            }
            
            // Build transitions
            for (let i = 0; i < paddedWord.length - order; i++) {
                const sequence = paddedWord.substring(i, i + order);
                const nextChar = paddedWord.charAt(i + order);
                
                if (!chain.has(sequence)) {
                    chain.set(sequence, new Map());
                }
                
                const transitions = chain.get(sequence);
                transitions.set(nextChar, (transitions.get(nextChar) || 0) + 1);
            }
        });
    });
    
    return { chain, startTokens, syllablePatterns, order };
}

// Advanced quality assessment with Yu-Gi-Oh specific criteria
function assessAdvancedQuality(text, isCompleteName = true) {
    ...
    for (const pattern of ENHANCED_MARKOV_CONFIG.forbiddenPatterns) {
    ...
    if (isCompleteName) {
        if (words.length < ENHANCED_MARKOV_CONFIG.minTotalWords) score -= 30;
        if (words.length > ENHANCED_MARKOV_CONFIG.maxTotalWords) score -= 20;
    }
    
    // Individual word quality
    for (const word of words) {
        const cleanWord = word.replace(/[^a-z]/gi, '');
        if (cleanWord.length < ENHANCED_MARKOV_CONFIG.minWordLength && cleanWord.length > 0) {
            score -= 15;
        }
        if (cleanWord.length > ENHANCED_MARKOV_CONFIG.maxWordLength) {
            score -= 10;
        }
        ...
        if (vowels === 0 && cleanWord.length > 2) score -= 25; // No vowels in long word
        if (consonants === 0 && cleanWord.length > 1) score -= 25; // No consonants
        
        // Check for excessive character repetition
        const repeats = cleanWord.match(/(.)\1{2,}/g) || [];
        score -= repeats.length * 20;
        
        // Check consonant/vowel clusters
        const consonantClusters = cleanWord.match(/[bcdfghjklmnpqrstvwxyz]{4,}/g) || [];
        const vowelClusters = cleanWord.match(/[aeiou]{3,}/g) || [];
        score -= (consonantClusters.length * 15 + vowelClusters.length * 10);
    }
    ...
    ENHANCED_MARKOV_CONFIG.commonWordEndings.forEach(ending => {
        if (words.some(word => word.toLowerCase().endsWith(ending))) {
            patternBonus += 8;
        }
    });
    
    // Yu-Gi-Oh style patterns
    const lowerText = text.toLowerCase();
    ENHANCED_MARKOV_CONFIG.cardNamePatterns.titleWords.forEach(title => {
        if (lowerText.includes(title)) patternBonus += 12;
    });
    
    ENHANCED_MARKOV_CONFIG.cardNamePatterns.descriptors.forEach(desc => {
        if (lowerText.includes(desc)) patternBonus += 8;
    });
    
    score += Math.min(patternBonus, 30);
...
// Intelligent word generation from chain
function generateFromAdvancedWordChain(chainData, maxAttempts = 8) {
    const { chain, startSequences, wordFrequency, positionStats, order } = chainData;
    
    let bestResult = '';
    let bestScore = 0;
    
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        const result = generateSingleWordSequence(chainData);
        if (!result) continue;
        
        const score = assessAdvancedQuality(result);
        if (score > bestScore) {
            bestScore = score;
            bestResult = result;
        }
        ...
        if (score >= ENHANCED_MARKOV_CONFIG.excellentQualityScore) {
            break;
        }
    }
...
function generateSingleWordSequence(chainData) {
    const { chain, startSequences, wordFrequency, positionStats, order } = chainData;
    
    if (startSequences.length === 0) return '';
    
    const result = [];
    let currentSequence = [...getRandomWeightedStart(startSequences, wordFrequency)];
    result.push(...currentSequence);
    
    let attempts = 0;
    const maxWords = Math.min(ENHANCED_MARKOV_CONFIG.maxTotalWords, 5);
    
    while (attempts < maxWords && result.length < maxWords) {
        attempts++;
        
        const key = currentSequence.join('|').toLowerCase();
        const transitions = chain.get(key);
        
        if (!transitions || transitions.size === 0) break;
        
        const nextWord = selectWeightedTransition(transitions, result.length, positionStats);
        
        if (!nextWord || nextWord === '<END>') break;
        
        result.push(nextWord);
        currentSequence = currentSequence.slice(1).concat([nextWord]);
        
        // Dynamic stopping based on length and naturalness
        if (result.length >= 2) {
            const stopProbability = Math.min(0.4, (result.length - 1) * 0.15);
            if (Math.random() < stopProbability) break;
        }
    }
...
// Smart character generation with better control
function generateFromSmartCharacterChain(chainData, maxAttempts = 10) {
    let bestResult = '';
    let bestScore = 0;
    
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        const result = generateSingleCharacterName(chainData);
        if (!result) continue;
...
function generateSingleCharacterName(chainData) {
    const { chain, startTokens, syllablePatterns, order } = chainData;
    
    if (startTokens.length === 0) return '';
    
    const numWords = Math.random() < 0.3 ? 1 : (Math.random() < 0.7 ? 2 : 3);
    const words = [];
    
    for (let w = 0; w < numWords; w++) {
        const word = generateSingleWord(chainData);
        if (word && word.length >= ENHANCED_MARKOV_CONFIG.minWordLength) {
            words.push(word);
        }
        
        if (words.length >= numWords) break;
    }
    
    return words.join(' ');
}

function generateSingleWord(chainData) {
    const { chain, startTokens, order } = chainData;
    
    let current = getRandomElement(startTokens);
    let result = current.replace(/\^/g, '');
    let attempts = 0;
    ...
    while (attempts < 20) {
        ...
        const nextChar = selectSmartCharacterTransition(transitions, result, current);
        
        if (!nextChar || nextChar === '$') break;
        
        // Handle word boundaries
        if (nextChar === ' ') {
            if (result.length >= ENHANCED_MARKOV_CONFIG.minWordLength) {
                break;
            } else {
        ...
        if (result.length >= ENHANCED_MARKOV_CONFIG.maxWordLength) {
            break;
        }
        
        // Quality check during generation
        if (result.length > 4) {
            const partialScore = assessAdvancedQuality(result, false);
            if (partialScore < 30) break; // Abandon poor quality
        }
    }
...
// Helper functions for weighted selection
function getRandomWeightedStart(startSequences, wordFrequency) {
    // Prefer sequences with higher-frequency words
    const weighted = startSequences.map(seq => {
        const weight = seq.reduce((sum, word) => 
            sum + (wordFrequency.get(word.toLowerCase()) || 1), 0) / seq.length;
        return { seq, weight };
    });
    ...
    for (const item of weighted) {
        random -= item.weight;
        if (random <= 0) return item.seq;
    }
    
    return getRandomElement(startSequences);
}

function selectWeightedTransition(transitions, currentPosition, positionStats) {
    const candidates = Array.from(transitions.entries());
    
    // Apply positional weighting
    const weighted = candidates.map(([word, count]) => {
        let weight = count;
        
        if (word !== '<END>' && positionStats.has(word.toLowerCase())) {
            const stats = positionStats.get(word.toLowerCase());
            const total = stats.total;
            
            // Boost words that commonly appear in current position
            if (currentPosition === 0 && stats.start > 0) {
                weight *= (1 + stats.start / total);
            } else if (currentPosition > 0) {
                weight *= (1 + (stats.middle + stats.end) / total);
            }
        }
...
function selectSmartCharacterTransition(transitions, currentResult, currentSequence) {
    const candidates = Array.from(transitions.entries());
    
    // Apply linguistic rules
    const weighted = candidates.map(([char, count]) => {
        let weight = count;
        
        // Prevent excessive consonant/vowel clusters
        const isVowel = /[aeiou]/.test(char);
        const lastChars = currentResult.slice(-3);
        
        if (isVowel) {
            const consecutiveVowels = (lastChars.match(/[aeiou]*$/)[0] || '').length;
            if (consecutiveVowels >= ENHANCED_MARKOV_CONFIG.maxConsecutiveVowels) {
                weight *= 0.1; // Heavily penalize
            }
        } else if (/[bcdfghjklmnpqrstvwxyz]/.test(char)) {
            const consecutiveConsonants = (lastChars.match(/[bcdfghjklmnpqrstvwxyz]*$/)[0] || '').length;
            if (consecutiveConsonants >= ENHANCED_MARKOV_CONFIG.maxConsecutiveConsonants) {
                weight *= 0.1; // Heavily penalize
            }
        }
        
        // Prevent excessive repetition
        if (currentResult.length > 0 && currentResult.slice(-1) === char) {
            const repeatCount = (currentResult.match(new RegExp(char + '*$'))[0] || '').length;
            if (repeatCount >= ENHANCED_MARKOV_CONFIG.maxRepeatedChars) {
                weight *= 0.05; // Almost eliminate
            }
        }
        
        // Boost natural word endings
        if (currentResult.length >= 4) {
            const potential = currentResult + char;
            for (const ending of ENHANCED_MARKOV_CONFIG.commonWordEndings) {
                if (potential.endsWith(ending)) {
                    weight *= 1.5;
                    break;
                }
            }
    ...
    for (const item of weighted) {
        random -= item.weight;
        if (random <= 0) return item.char;
    }
    
    return candidates[0][0]; // Fallback
}

// Main improved Markov generation function
function generateByImprovedMarkov() {
    // Try word-level generation first (more coherent)
    if (Math.random() < 0.7) {
        const wordChain = buildAdvancedWordChain(cardNames, 2);
        const wordResult = generateFromAdvancedWordChain(wordChain);
        
        if (wordResult && assessAdvancedQuality(wordResult) >= ENHANCED_MARKOV_CONFIG.minQualityScore) {
            return formatCardName(wordResult);
        }
    }
    
    // Try character-level with smart controls
    const charChain = buildSmartCharacterChain(cardNames, 3);
    const charResult = generateFromSmartCharacterChain(charChain);
    
    if (charResult && assessAdvancedQuality(charResult) >= ENHANCED_MARKOV_CONFIG.minQualityScore * 0.8) {
        return formatCardName(charResult);
    }
    
    // Final fallback: try simpler word-level
    const simpleWordChain = buildAdvancedWordChain(cardNames, 1);
    const simpleResult = generateFromAdvancedWordChain(simpleWordChain);
    
    if (simpleResult && assessAdvancedQuality(simpleResult) >= 40) {
        return formatCardName(simpleResult);
    }
...
// Batch generation with quality guarantee
function generateQualityMarkovBatch(count) {
    const results = [];
    const attempted = new Set();
    let attempts = 0;
    const maxAttempts = count * ENHANCED_MARKOV_CONFIG.maxGenerationAttempts;
    
    while (results.length < count && attempts < maxAttempts) {
        attempts++;
        
        const candidate = generateByImprovedMarkov();
        if (!candidate) continue;
        
        const lowerCandidate = candidate.toLowerCase();
        if (attempted.has(lowerCandidate)) continue;
        attempted.add(lowerCandidate);
        
        if (isValidName(candidate)) {
            const quality = assessAdvancedQuality(candidate);
            if (quality >= ENHANCED_MARKOV_CONFIG.minQualityScore) {
    ...
    results.sort((a, b) => assessAdvancedQuality(b) - assessAdvancedQuality(a));
    ...
    function generateSingleName() {
        switch(currentMethod) {
            case 'wordSplit': return generateByWordSplit();
            case 'syllableSplit': return generateBySyllableSplit();
            case 'thematic': return generateByThematic();
            case 'markov': return generateByImprovedMarkov();
            default: return generateByWordSplit();
        }
    }
    ...
    function generateUniqueNames(count) {
        ...
        while (uniqueNames.length < count && attempts < CONFIG.maxAttempts) {
            const newName = generateSingleName();
            attempts++;
            
            if (isValidName(newName)) {
                uniqueNames.push(newName);
                generatedNamesSet.add(newName.toLowerCase());
            }
        }
        
        if (uniqueNames.length < count) {
            setStatus(`Generated ${uniqueNames.length} unique names (requested ${count}). Some duplicates or forbidden words were filtered out.`);
        }
    ...
    function displayResults(names) {
        if (names.length === 0) {
            elements.results.textContent = 'No valid names generated. Try changing generation method or adding more sample data.';
            elements.copyAllBtn.style.display = 'none';
            return;
        }
        
        names.sort();
        elements.results.textContent = names.join('\n');
        elements.copyAllBtn.style.display = 'block';
    }

    function generateCardNames() {
        elements.spinner.style.display = 'inline-block';
        setStatus('Generating new card names...');
        
        const numToGenerate = Math.min(parseInt(elements.numNames.value) || 10, CONFIG.maxNames);
        
        if (numToGenerate > CONFIG.performanceWarningThreshold) {
            generateInChunks(numToGenerate);
        } else {
            setTimeout(() => {
                const generatedNames = generateUniqueNames(numToGenerate);
                displayResults(generatedNames);
                elements.spinner.style.display = 'none';
                setStatus(`Generated ${generatedNames.length} unique card names`);
            }, 100);
        }
    }

    function generateInChunks(totalToGenerate) {
        const allGeneratedNames = [];
        let processedCount = 0;
        
        function updateProgress() {
            const progress = Math.min(processedCount, totalToGenerate);
            setStatus(`Generating names... ${allGeneratedNames.length} unique names created (${progress}/${totalToGenerate} attempts)`);
        }
        
        function processChunk() {
            const remainingToGenerate = totalToGenerate - allGeneratedNames.length;
            const currentChunkSize = Math.min(CONFIG.chunkSize, remainingToGenerate);
            
            if (currentChunkSize <= 0 || processedCount >= totalToGenerate * 2) {
                displayResults(allGeneratedNames);
                elements.spinner.style.display = 'none';
                setStatus(`Generated ${allGeneratedNames.length} unique card names`);
                return;
            }
            
            const chunkNames = generateUniqueNames(currentChunkSize);
            allGeneratedNames.push(...chunkNames);
    ...
    elements.sampleBtn.addEventListener('click', () => {
        cardNames = [...SAMPLE_CARD_NAMES];
        generatedNamesSet.clear();
        setStatus(`Reloaded ${cardNames.length} sample card names`);
    });
    
    elements.generateBtn.addEventListener('click', generateCardNames);
    ...
    elements.copyAllBtn.addEventListener('click', () => {
        const text = elements.results.textContent;
        navigator.clipboard.writeText(text)
            .then(() => {
                elements.copyAllBtn.textContent = 'Copied!';
                setTimeout(() => {
                    elements.copyAllBtn.textContent = 'Copy All Names';
                }, 1500);
            })
            .catch(() => {
    ...
    elements.numNames.addEventListener('input', () => {
        const numNames = parseInt(elements.numNames.value) || 0;
        elements.performanceWarning.style.display = 
            numNames > CONFIG.performanceWarningThreshold ? 'block' : 'none';
    });
    ...
    setStatus(`Ready to generate! Using ${cardNames.length} sample card names`);
});